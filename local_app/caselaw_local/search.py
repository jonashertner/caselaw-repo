...
def search(
    ...
    *,
    ...
    if not q.strip():
        ...
        rows = conn.execute(
            f"""
            SELECT id, source_name, canton, level, language, docket, decision_date, published_date, title, url, pdf_url,
    ...
    rows = conn.execute(
        ...
        SELECT d.id, d.source_name, d.canton, d.level, d.language, d.docket,
               d.decision_date, d.published_date, d.title, d.url, d.pdf_url,
...
def get_doc(conn: sqlite3.Connection, doc_id: str) -> Optional[Dict[str, Any]]:
    row = conn.execute(
        ...
        SELECT id, source_id, source_name, level, canton, court, chamber, language, docket,
               decision_date, published_date, title, url, pdf_url, content_text, fetched_at, updated_at
        FROM decisions
        WHERE id = ?
        LIMIT 1;
        """,
